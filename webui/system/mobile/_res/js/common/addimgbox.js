 ...
 * ex: 
    ...
    <!-- HTML -->
    <form class="content item upimg-item" method="post" enctype="multipart/form-data">
        <div class="addface">
...
XLJ.Addimgbox.prototype = {
    ...
    fail: function(target) {
        if (!target || !target.length > 0) return XLJ.tip('上传失败，请重新操作')
    ...
    file: function(target, callback, parent) {
        ...
        if (callback) callback(_thisNo)

        /* _parent.attr('class', '')
         _img.show()

        if (!_parent.hasClass('haveimg')) {
            _parent.addClass('haveimg')
            root.photo.addnewbtn(parent, Number(_thisNo) + 1)
        }*/
    },
    ...
    add: function(target, callback) {
        ...
        var _this = target,
            ...
            _parent = _this.parentsUntil(root.itemElName).last().parent(),
        ...
        XLJ.ajaxSubmit(_parent, _progressBox, function(response) {
            ...
            var option = {
                ...
                pictureLocalUrl: response.result.url
            }
